...
| 12  | **Retirement, not deletion.** When a row disappears (or a rename produces a new directory) the root is marked retired: it drops out of every future session's `cwd`/`additionalDirectories` immediately, but nothing on disk is removed. At idle sweep each of its session worktrees goes through the existing safe cleanup (`removeSessionWorktree`: a dirty tree or unique commits ⇒ retained and reported); the clone itself is removed only once no worktree remains and the same dirty/unique-commit checks pass on it, otherwise it is retained and reported. A re-authorized repository un-retires in place.                                                                                                                                         | A retired root can still be a live session's `cwd`, and a shared checkout can hold unsaved work; unconditional subtree removal would break the running runtime or discard work that the per-worktree rules already promise never to auto-delete. One cleanup rule for worktrees and clones alike.                                                                    |
...
and drops it from every future session immediately. Its session worktrees are
cleaned at idle sweep by the existing dirty/unique-commit rules; the checkout is
removed only once no worktree remains and it is itself clean, otherwise it is
retained and reported. Re-adding the row un-retires the root in place.
