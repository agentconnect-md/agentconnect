...
> materialization below is implemented (decisions 13–20): `always`,
...
| 6   | **Cross-repository review = the same-repository review with the root swapped.** `prepareGithubReviewWorkspace` resolves the hook repository to a root; found ⇒ `fetchReviewRevision(root, …)` + exact worktree as `cwd`, prompt unchanged ("Trusted review workspace"); not found (grandfathered hook, clone failure) ⇒ today's revision-only fallback, unchanged. A repository with no row whose account a GitHub installation grant covers resolves to a root placed and attested as a row's, under the hook's repository id, and reviews exactly like an `on-demand` row: that review's `cwd` alone, resumed from its checkout's attestation while the grant covers it, retired like any root no `always` row names (decision 12).                                                                                                                                                                                                               | With the hook gate requiring workspace ∪ authorized, every legitimately configured review lands on a root; the fallback shrinks to a safety net.                                                                                                                                                                                                                     |
...
> **Status:** implemented. Decisions 13–20 extend the design above, and every
> step of the change map below has landed: the `materialize` column with its
> REST surface and projection, installation grants, and the selector on the
> daemon (`repo-selector-v1`), behind a Control Plane gate on the agent's
> selector and that feature, with **By decision** and the **Repository
> selector** in the console.
...
| 15  | **The selector is a built-in Decision consumer at workspace preparation.** Its candidates are the rows marked `decision` plus the rosters of the installation grants marked `decision`. The agent chooses the evaluator once, `repositorySelector: { providerId, model }`, as the **Repository selector** under Additional repositories in Edit workspace, through the Decision editor's Provider · model picker limited to models that answer Choice questions, shown as soon as any authorization is marked `decision` or a selector is set; the daemon generates the question. Candidates are split into chunks of at most 31, each chunk one Choice question of those repositories plus a `none` option, evaluated concurrently within the evaluator's own caps. Following [decisions.md §1](decisions.md), the consumer lives here, in the feature that owns the action; the typed answer is evidence, and the `materialize` choice is the authority that turns it into clones. | A saved Decision carries a fixed question of at most 32 options; the candidate list is the agent's and changes with its grants, so the question must be built from it. Chunking is what makes a few hundred candidates one bounded stage rather than one evaluation per repository.                                                                                   |
| 16  | **State is the model-selection state.** The selector reads the same input as [Agent runtime and model selection](decisions.md#106-agent-runtime-and-model-selection): a chat's opening message with bounded history, or the shared code-host event, subject, observed history and optional PR/MR context, plus `workspace.primary` naming the primary repository. Candidates appear only in the question's criteria, never in the state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | One state builder for both once-per-session consumers, one set of budgets and truncation rules, and the request stays within the 32,000-byte estimated token budget and the evaluator's 32 KiB hard limit.                                                                                                                                                            |
...
| 18  | **No fallback.** When any authorization is marked `decision`, a missing precondition fails the session's start with a visible error naming the cause, through the same path a runtime that cannot start uses: an agent with no `repositorySelector`, an evaluator the catalog does not offer for Choice questions or that is not ready (`DecisionReadiness` other than `ready`), a Control Plane that cannot answer the roster a `decision` grant needs, or an `unavailable` evaluation of any chunk. The daemon never treats a failed selection as `always` or as `on-demand`. The Control Plane accepts `decision` on a row or grant only while the agent has a `repositorySelector` and every daemon that would run it advertises `repo-selector-v1` (409 otherwise), and refuses to clear the selector while any entry uses it; the console disables **By decision**, saying why, while no provider is ready where the agent runs or no selector is set.                         | A silent downgrade would make which repositories a session stands in depend on provider health that nobody can see. The operator chose by decision; a failure of its precondition is theirs to see and fix, exactly as a runtime that cannot start is.                                                                                                                |
...
selectors reuse one collected code-host snapshot. A chat with no recorded opening
uses the message text alone; an unsupported code-host input fails repository selection
visibly. The stage runs where the model-selection evaluation runs: on the serving
...
3. **`decision`** — the evaluator pair on the agent, the roster request and
   ...
   demand. _Landed:_ the Control Plane gate — `decision` on a row or grant
   (`POST` and `PATCH`) needs the agent's `repositorySelector` (409
   `REPOSITORY_SELECTOR_MISSING`) and the agent's daemon, or every ready member
   of its group or the pool, advertising `repo-selector-v1` (409
   `DAEMON_FEATURE_MISSING`), and clearing the selector is 409 while any entry
   is `decision`; and the console — **By decision** on each row, grant, add
   step and authorize step, disabled with its reason while no Decision
   provider is ready where the agent runs or no selector is set, and the
   **Repository selector** in Edit workspace, saved through agent `PATCH`.
