...
A native-writable home (the daemon's own disk, a sandbox shim, an older Control Plane) serves the same three operations through the compatibility writer's guarantees, so the console and the model tools no longer depend on where the tree lives. The revision precondition is checked and the one file replaced under the per-directory lock every daemon-side writer takes, with the filesystem's absent/mtime guard against an out-of-band edit, and the stored text is normalized and stamped as every managed write is; that is what `conditional`, `exactCreate` and `exactEdit` promise here. Delete keeps the same fence through the removal itself, so it is advertised only where the port can verify the file immediately before unlinking (`rmIfMatch`: the daemon's own disk today); a sandbox shim or an older peer offers create and update without delete rather than an unconditional removal. What this path does not offer: the index and the change log follow the write rather than joining it in one commit (the index is derived and regenerated on the next write regardless), the receipt is minted in process and kept nowhere, so a lost answer is reported as uncertain and never replayed, and there is no source-turn capture fence, exactly as for a compatibility write on such a home. It does not emulate `atomicTransaction`; a home that has the primitive takes the transactional path.
