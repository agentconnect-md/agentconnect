...
| Skills                | The `agentconnect-platform` skill enabled by default — platform introduction plus admin guidance honoring webchat-preset-agentconnect-mcp.md invariants 5/10: use the AgentConnect MCP toolset when the session has it; REST only with a credential provisioned outside the conversation (agent env config); otherwise direct the user to the console — never solicit a key in-session. Provisioning registers an ordinary org skill source named `agentconnect` (the platform monorepo's `skills/` dir, pinned by numeric GitHub repo id) in the same transaction; if the org already owns that source name, the source is left untouched and the preset ships without default skills. Both stay ordinary editable resources |
