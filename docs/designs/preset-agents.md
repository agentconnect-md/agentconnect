...
| Skills                | **Whatever `agentconnect-md/agentconnect-skill@main` exposes** — provisioning registers an ordinary org skill source named `agentconnect` (that repo's `skills/` dir, pinned by numeric GitHub repo id) in the same transaction, with an EMPTY filter, and enables it on the preset whole (`agentconnect/*`). The repo's head is the roster: publishing a skill there reaches every install with no CP release, and neither half may be re-pinned (a source filter and a specific-skill enable-ref each undo the other's openness). Orgs born before this were widened by migration `20261008000000`, so new and existing installs resolve identically. Today the dir ships `agentconnect-platform` — platform introduction plus admin guidance honoring webchat-preset-agentconnect-mcp.md invariants 5/10: the agent executes admin calls only through the AgentConnect MCP toolset when a session has it; with no admin tools it never calls the REST surface itself under any credential (invariant 10 rejects static user-key authority however provisioned) and instead directs the user to the console or helps them drive the REST API as their own client — a key never enters the conversation — and `agentconnect-create-agent` — template-driven agent creation (first template: `code-reviewer`): it runs only where the session carries the admin MCP toolset (otherwise it sends the user to the console webchat), collects prerequisites and parameters through elicitation cards, then calls `createAgent` — which in webchat opens the console create dialog prefilled with them, so the user presses Create and the skill hands back the console-only follow-ups. If the org already owns the `agentconnect` source name, the source is left untouched and the preset ships without default skills. Both source and agent stay ordinary editable resources |
