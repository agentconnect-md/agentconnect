...
| Start your first conversation | any session exists (2026-07-30: a session existing at all means a conversation has been driven here — Playground or channel; requiring a terminal status made orgs with live sessions re-run a chat just to clear the step). Client-derived today from the caller-visible `GET /sessions` list, so restricted/private-only orgs under-report until the §6.3 org-level endpoint carries the exact signal |
