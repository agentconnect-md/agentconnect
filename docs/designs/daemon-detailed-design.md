...
Draining is the existing SIGTERM path — gate new turns, await in-flight ones to `limits.shutdownDrainMs` (`limits.poolShutdownDrainMs` under `--k8s`, where a duty-holding member also releases each held group with an acknowledged `duty/release`; k8s-daemon-pool.md §12), cancel stragglers — which is what a `preStop` hook and a SIGTERM both reach. **`terminationGracePeriodSeconds` must exceed that budget**, or the kubelet SIGKILLs mid-drain and the graceful window is a promise the deployment cannot keep. The daemon cannot read its own grace period (it has no pod read access), so at startup in cluster mode it logs the drain deadline it will actually use and leaves the alignment to the deployment.
