...
| Every console workspace read needs a BOUND sandbox: with no channel the panels answer "absent" rather than waking the pod                                                                            | The console re-reads status on each session page view, so waking on a read would resurrect a suspended pod on every visit and fight the idle sweep. Git already behaved this way, and the two surfaces agreeing beats one of them being quietly more eager.                                                                                                                                                                                                                                                                                                  |
