...
| The bootstrap upgrade path is skipped outright                                     | A cloud daemon's version is its image: there is no CLI or version store to self-install from, so the path is short-circuited rather than left to fail on a missing `cli-entry` pointer. |
