...
Legacy shared-VM microsandbox sessions keep the agent's host until they retire. A session-bound host keeps the cwd it launched in, so a turn prepared elsewhere (a formal review's own checkout) relaunches it there. A conversation-scoped interrupt fences only the agent's shared host and the interrupted sessions' own, so other sessions can still start theirs; an agent-wide interrupt, or a backstop falling back to stopping every host, fences them all. OpenCode-lineage `acp` launches carry `--port 0` unless they name a port, since a global `server.port` would pin every process to one listener.
