...
- **Linux (systemd, system scope):** `/etc/systemd/system/agentconnect.service` (a named instance is `agentconnect@<name>.service`), with `[Service] ExecStart=execPath cliEntry run --root <root>`, `User=<the daemon account>`, `WorkingDirectory=` + `Environment="HOME="` for that account, `Restart=always`, `Environment="AGENTCONNECT_ROOT=<root>"`, and `[Install] WantedBy=multi-user.target` — a `--user` unit dies with the last login session when `Linger=no`. Run `systemctl daemon-reload` after writing. Install also `enable`s the unit and writes `/etc/polkit-1/rules.d/49-<unit>.rules` scoped to that unit and account — or, without polkit's rules.d backend, a visudo-checked `/etc/sudoers.d` grant that `up`/`down` use through `sudo -n` — so `up = start` and `down = stop` need no password; only `install-service` / `uninstall-service` re-exec through sudo. Status uses `is-active` and `show -p MainPID`. Legacy `~/.config/systemd/user` units remain drivable in their own scope and are retired (unelevated) by the next `install-service`. See cli-daemon-split.md §4.2.2.
...
| No SRT/bwrap mechanism detection; the agent's strategy has no local effect                                                                                                                                                               | The pod is the isolation unit. `sandbox.host: false` therefore refuses startup with an explanation rather than silently meaning something else.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
```jsonc
{
  ...
  // ---------- Execution strategies and shared host directories ----------
  "sandbox": {
    // Every strategy is offered by default; its startup probe decides whether it is available.
    // false withdraws one; microsandbox also takes { image, cpus, memoryMiB, diskGiB }.
    "host": true,
    "srt": true,
    "microsandbox": true,
...
Each agent names the strategy its sessions run in (`host`, `srt` or `microsandbox`), and the
daemon offers a table of them ([session-executors.md](session-executors.md) §5).
...
isolated HOME. A session of such a runtime in a sandboxing strategy is refused,
and on a machine that offers no `host` its admission probe fails loudly too, so
the runtime is not advertised on hosts that demand confinement. Agent
configuration cannot set the flag; it lives on the daemon/registry-owned runtime
definition alongside `readRoots`.
...
A runtime whose `sessionMcpServers` scope is `'per-process'` accepts the list but keeps it in state every session of the process shares: OpenCode, and its fork Kilo, registers each server on the working directory's instance by name, closing the one an earlier session registered there. On a shared host every session's tool calls would carry the bridge token of whichever session registered last — running under that session's `SessionContext`, or failing with `this agent turn has been stopped` once its turn has ended. The daemon therefore gives each chat session of such a runtime its own ACP host (a session-bound host key, as a microsandbox session has) in the session's usual workspace; internal passes stay on the agent's host. The scope is audited per runtime id in `runtimes/session-mcp-servers.ts`, and a RuntimeDef declaration, `'per-session'` included, overrides it.

Legacy microsandbox sessions keep their original shared VM and HOME so runtime history remains resumable, but a `'per-process'` runtime still gets a separate ACP host for each session. Retiring one session stops only its host; the shared VM survives until its last legacy session retires. Internal memory-distillation and commit-message passes create their own ACP session before reading that session's permission modes, then require a successful read-only/plan switch before prompting. They do not depend on a chat session having warmed the agent host's option cache.
...
If S_A closed by TTL, use `session/load`; on failure, start a new session but still replay transcript for continuity. Ordinary catch-up leaves out the agent's own messages, which its session already holds; a recreated session holds none, so its first replay also includes the agent's own replies, marked `[you]` under a heading saying the session restarted without its history.
