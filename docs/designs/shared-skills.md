...
- **Install from skills.sh** opens a search modal over the public
  ...
  registers it as an ordinary source whose `source` is the hit's `owner/repo`
  and whose `skills` filter is exactly the one skill picked. One tile therefore
  means one skill on this path, matching how the registry is browsed, while the
  GitHub path still registers a whole repository. Hits already covered by an
  existing source are shown as added rather than offered again, and the library
  name defaults to the skill name, de-duplicated so it cannot collide with a
  name agents already bind to (which the create route refuses with 409).
...
| control-plane | `prisma/schema.prisma` for the `SkillSource` registry and `Agent.skills`; new `http/routes/skill-sources.ts` + `http/skills-registry.ts` (skills.sh search proxy); `orchestrator/` to resolve sources into `AgentSpec` and fan out `agent/upsert` after a source change; `github/` for Trees API preview scanning                         | Section 4                                                                                                    |
...
| web           | `ToolsHubView.tsx` without mocks; new `modals/ImportSkillSourceModal.tsx` replacing `AddSkillModal`; `console/InstallRegistrySkillModal.tsx` for the skills.sh search path; `lib/api.ts`; `lib/data-context.tsx`; agent editor                                                                                                            | Section 7                                                                                                    |
