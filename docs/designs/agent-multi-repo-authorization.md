...
`AgentRepoAuthorization` gains the same column, `materialize RepoMaterialization
@default(always)`, over a new enum `RepoMaterialization { always, decision,
on_demand }`; a grant's value is validated to the two it may take
...
5. **Installation grants (decision 10)** are consulted after the explicit rows
   ...
   explicit row matched earlier keeps its own tier.
...
| `POST /orgs/:orgId/agents/:agentId/repos`               | `denyViewerWrite` + agent `canEdit`                             | Body `{ owner, repo, access, materialize? }` (`materialize` defaults to `always`). Require a live installation claimed by the organization, resolve `repoId`, run decision 8 identity attestation, and enforce unique `(agentId, repoId)`. A GitHub App workspace cannot explicitly reauthorize its implicit repository. Scratch may authorize any covered repository. A manual GitHub workspace may explicitly authorize only its own workspace repository for Control Plane-owned review and Checks.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `PATCH /orgs/:orgId/agents/:agentId/repos/:repoAuthId`  | Same                                                            | Body `{ access?, materialize? }`. Access permits only monotonic `read -> comment -> write`; downgrade returns 409 and requires delete plus regrant to make review and Check cleanup explicit. `materialize` moves freely among `always`, `decision` and `on-demand`; leaving `always` retires the root in place ([multi-repository-workspaces.md](multi-repository-workspaces.md), decision 13). Re-run identity attestation, requiring write for write, require a live owner installation, and audit `agent_repo_change`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `POST /orgs/:orgId/agents/:agentId/installations`       | `denyViewerWrite` + agent `canEdit` + organization role `owner` | Body `{ installationId, access, materialize }`. Require a live installation claimed by this organization; default access `read`, default `materialize` `on-demand` (`always` is rejected); enforce unique `(agentId, installationId)`; audit. No per-repository identity attestation (decision 10). A non-owner receives 403 with a message naming the role, not 404.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `PATCH /orgs/:orgId/agents/:agentId/installations/:id`  | Same                                                            | Body `{ access?, materialize? }`. Access follows the repository rows' monotonic rule; `materialize` may move either way between `decision` and `on-demand`, and `always` is rejected. Re-projects the spec and audits.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
installation and matches it against the grants after the explicit rows.
...
- **Installation grants:** owner-only create and delete (403 for a
  ...
  grant's `always` rejected; a row leaving `always` retiring its root and
