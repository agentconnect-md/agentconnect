...
A session pod uses its own stored activity even when this member has no host for
it. A sibling session's traffic or dispatch cannot keep it awake. Local admission
is checked again after activity reads and watcher queries; a new turn holds its
own pod. Debug logs report the launch count and why each candidate was skipped.
...
| Model-session host start under a key server, and a review host's cold start with a pre-prepared cwd, prepare the agent's own checkout with no session request | per cold host start                                                             | wakes the agent pod; pure overhead                             | done: a cold start follows the session's own preparation, lists its clones, and re-verifies only the session pod's skills          |
