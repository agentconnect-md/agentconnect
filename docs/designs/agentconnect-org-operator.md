...
| `reconcile/rollout.ts`        | Operator half only: conditioned patch for Suspended, drain annotation for Running. Until the daemon-side consumer (sandbox watch + drain-to-Suspended) lands, Running instances stay `pending`; drain timeouts → `failed` also TODO. | partial       |
