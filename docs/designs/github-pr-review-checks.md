...
Wire/schema retain `required` and `status` enum values for rolling compatibility, but they are not delivered capabilities. HTTP create/update returns a semantic conflict for both, and daemon/worker fail closed again. R2a accepts `check_run.rerequested`, first-run `check_run.requested_action(request_review)`, and GitHub reviewer UI `pull_request.review_requested`. All three may start a new generation only for a current **informational** hook, and the explicit requester must still have repository write/admin permission. Required-safe semantics remain R2b.
...
Informational Checks:   AgentConnect PR Review
Required status:        agentconnect/status/<hookId>
Informational status:   agentconnect/info/status/<hookId>
```

R2a informational name is fixed human-readable label; console does not manage it as required, and operator must not pin it manually in branch protection. Correlation uses `external_id + write marker`, keeping internal hook ID out of final `name`/`output`. Fixed label means multiple informational hooks on same SHA may share display name; R2a accepts this display-only limitation. Required context remains unique/immutable and pins expected source to AgentConnect App. required→informational/off, disable, HookDef/Agent delete first tombstone + queue non-passing cleanup for current required context, then informational uses its own name; console warns operator to remove branch rule. Transport/gate switches change context and never implicitly reuse.
...
- `external_id=projection.id` is recovery correlation, **not GitHub idempotency**. For rolling upgrade/rollback, create POST retains legacy recovery name `agentconnect/info/review/<hookId>`. After persisting `checkRunId`, name-only PATCH changes to human label, and every later state PATCH also includes label. Ambiguous create lists runs by report SHA + legacy name + App with pagination, then matches `external_id + write marker`; DB lease/CAS serializes creates. Failure of name-only PATCH does not roll back settled projection; later state PATCH reconverges name. R2a does not bulk-rename historical settled Checks.
...
`skipped` informational Check titled `Review requires a maintainer request` with a
`Request review` requested action. This HookRun failure is a durable control anchor
...
- **Informational Check request/rerequest is implemented.** After verifying signatures for `check_run.rerequested` and `check_run.requested_action` identifier `request_review`, relay sends only `{checkRunId, repoId, headSha, deliveryKey}` metadata to CP. CP reverse-resolves durable projection, verifies current `check + informational` hook, and returns metadata-only dispatch command. First-run requested action explicitly requests base SHA persisted on current HookRun. During rolling upgrade, old `rerequested` derives base SHA from signed same-repository PR association. Relay realigns current compiled rule, rechecks sender write/admin permission, and dispatches new delivery key review generation reusing/updating that Check Run. It bypasses ordinary subscription cadence, and existing daemon/HookRun dedup absorbs same GitHub delivery GUID.
...
`GithubInstallation.permissions Json` exists; unknown `{}` fails closed. Setup callback, Sync, and installation doorbell pull (including `new_permissions_accepted`) replace local facts with installation-effective permissions returned by GitHub. Selected repository/projection remains and dynamically refreshes provenance; console displays exact `checksPermission`. With `checks:write`, GitHub App automatically receives `check_run.rerequested` and `check_run.requested_action`; relay handles only rerequested and `request_review`, returning 202 no-op for other actions.
