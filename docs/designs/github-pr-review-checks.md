...
- `latestReviewVerdictRun(hookId, pullNumber, reportSha)` is the sealed-verdict lookup behind both the `hook/start` amendment and the `review-authorize` gate for `review_action_only` runs; a reconciliation of the run's own reserved attempt skips the gate it already passed.
