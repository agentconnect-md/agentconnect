...
3. **R2b—required-safe gate (future):** only hooks that unconditionally cover PR head and pass real fork/branch-protection/rerequest tests can be required-eligible. Ordinary conversational turns cannot write this context; a verdict amendment (decision 1, item 5) is admitted there together with same-SHA rerequest acceptance, not before.
...
5. Enforce `APPROVE ⇒ pass`, `REQUEST_CHANGES ⇒ fail`; only `COMMENT` accepts explicit `pass|fail|neutral`. One successful review action per turn. On the same revision, `review_action_only + COMMENT + neutral` means the turn did not alter code-review verdict and cannot overwrite existing `pass/fail`. Only generation-native neutral or a later explicit `pass/fail` changes Check projection.
   - **Verdict amendment (2026-09-26).** A `review_action_only` PR turn — an unmentioned PR-conversation comment or an inline review-thread reply — holds review authority only as an amendment of a **sealed** verdict, and the CP names that verdict on the turn's `hook/start/ok` (`amendment: {reportSha, event, verdict}`): this hook's latest verdict on the turn's own revision under its current repository binding and projection epoch (`latestReviewVerdictRun` — the latest generation, or a later amendment, within the projection key `(hookId, repoId, reportSha, projectionEpoch)`; a verdict a rebind or re-enable retired never qualifies), submitted by a turn that has ended, and judged on the started row because an `issue_comment`'s accepted row carries no revision yet. No verdict, a generation still running, or a generation that ended without a verdict names nothing; that is `Request review`'s job. The daemon exposes `submitGithubReview` on such a turn only when the named `reportSha` equals the turn's trusted head, and `github/review-authorize` reruns the same eligibility and additionally requires the requested verdict to **change** the sealed one (`pass ↔ fail`, or settling `neutral`; `neutral` itself is never an amendment), so a thread cannot accumulate repeated same-verdict reviews. Every other gate is unchanged: the policy ladder caps the event, the broker rereads the PR and fences head/base, and the run's projection converges as a fresh terminal generation with no `queued`/`in_progress` phase — an amendment never has an intermediate state to retract. The turn keeps its conversational worktree: the daemon fetches no exact checkout, and the prompt block (`Verdict amendment available`) tells the model the thread's claims are untrusted and must be verified with revision-addressed reads before the verdict moves. An inline reply still posts to its thread beside the amendment; only the top-level fallback comment stays mutually exclusive with a submitted review. The permission bar is the trigger gate every thread comment already crosses at the relay (a trigger-authorized role or a Trusted user, and for an unmentioned comment the PR author too), which is the same gate `Request review`, a Check rerequest, and workflow approval use: an amendment is the shortcut for "comment, then rerequest", not a new audience. On a pull request whose author holds no trigger role and is not a Trusted user, an amendment can therefore only come from a maintainer's reply that mentions the Agent or App inside an inline review thread: an unmentioned reply fails the author half of the gate, and a mentioned top-level comment opens a generation instead. An amendment never dismisses the earlier formal review on GitHub: under a `request_changes` policy it can only record `COMMENT + pass`, so the Check turns green while the App's earlier change request stays on the pull request until a maintainer dismisses it — the same state a rerun leaves today.
...
The prompt side of this boundary is split by lifetime (2026-09-02). Everything that holds for every delivery of one session — the daemon exclusively owns the reply, no direct comment or review writes through `gh` or the API, other GitHub tools are read-only, how a review generation is recorded with `submitCodeReview` and an inline thread with `replyGithubReviewThreads`, when local files may be trusted — is a `# GitHub` block on `NormalizedMessage.standingContext`: persisted with the logical session, read once on the system-prompt channel (or as the first prompt block), never a transcript row. Each delivery's text keeps only its own facts: the trusted header, the fenced body, one line naming what this delivery is (review generation with the policy's verdict events, inline thread, plain conversation) and a one-clause reminder that the daemon owns the reply. The review orchestrator's per-revision workspace block is unchanged: the SHAs and the checkout verdict are the delivery's, not the session's. That per-delivery text is the turn body's `prompt` (transcript-full-tool-body.md §9); the transcript row's `text` is the console's short form — a comment verbatim, otherwise one line such as `Opened PR #42 · title` — and the orchestrator appends its block to the prompt seat alone. The block opens by scoping every rule to a turn opened by a delivery, because a hook-origin session can be continued from the console (webchat-cross-integration-continuation.md §9), where no poster runs: such a turn is answered in the session, the daemon posts nothing for it, and the agent's code-host tools act at its own discretion.
...
HookRun additionally stores `projectionId/projectionGeneration`, `projectionEpoch`, `projectionIntent`, policy/report snapshots, `pullNumber/head/base/reportSha`, relay-ingest `startedAt`, daemon-preparation `preparingAt`, daemon-dispatch `turnStartedAt`, TEXT `reviewId`, `reviewAttemptId/reviewAttemptState`, `reviewEvent/verdict`, body-free fallback `publishedCommentKind/publishedCommentId`, and dispatch fence. Projection is GitHub commit/context granularity; subject rows hold PR revision. Unique key + transactional `generation` increments ensure one external current generation per hook/repository/report SHA. Every worker job carries `(projectionId, generation)` CAS; old epoch/generation can update history only.

Immediate reporter kick reduces latency; periodic scan supplies reliability. Before each claim, worker scans HookRun to repair crash windows where lifecycle mutation committed but projection convergence did not, then consumes projection outbox. Durable HookRun reconstructs any process gap around `accepted`, `hook/preparing`, `hook/start`, `github/review-result`, or completion:
...
- `rc/run-report(accepted)` with authoritative revision always transactionally creates/updates HookRun. R2a opens a `queued` projection generation only for `projectionIntent=revision_event` on open, synchronize, or an edited PR with trusted `baseChanged=true`; a delayed accepted callback derives any later durable phase instead. If previous external effect remains in flight, store only `pendingIntent` until remote confirmation. Label events, unmentioned comments, draft/ready transitions, and other `review_action_only` turns do not create or advance a projection at accepted/start; only a successful structured review action — a verdict amendment (decision 1, item 5) — establishes an informational generation and terminal verdict. For a PR-conversation comment that explicitly mentions the Agent/App and passes the permission gate, relay forwards `explicitReviewRequest=true` only when the accepted rule enables formal reviews (the body still never enters CP), and the comment then opens a generation as a `revision_event`. With review policy `off`, the same mention still dispatches through its configured trigger but remains `review_action_only`; this policy does not select or alter the reply/output path. `check_run:rerequested`, `check_suite:rerequested`, `check_run:requested_action`, `pull_request:review_requested`, and an authorized `workflow_run:in_progress` dispatch also produce `revision_event` and a new same-SHA generation. Automatic external-author PR revision events do not dispatch and converge to a failing `review_request_required` projection with a requested action. PR `issue_comment` without SHA creates HookRun only; review action waits for daemon `hook/start` authoritative revision.
- When the daemon begins formal-review workspace preparation, it sends best-effort metadata-only `hook/preparing`. The current accepted `revision_event` generation advances to internal `preparing`, published as GitHub `status=in_progress` with `Preparing review`. The workspace work does not wait for this status RPC. No new session link is added until a session ID is recorded.
- After the session exists, `hook/start` remains the acknowledged prompt barrier. It records `turnStartedAt` and advances the current generation to internal `in_progress`, published as `Analyzing this revision`.
...
- Seal is repository-level CAS independent of coordinator's stale snapshot: `queued/preparing/in_progress` writes only when `sealedThrough < generation`; terminal write seals in same transaction. `setProjectionDesired(currentHookRunId)` locks/rereads durable HookRun and recomputes authoritative desired from its lifecycle and verdict, so delayed accepted/preparing/start edges cannot regress the current phase or overwrite a submitted formal verdict.
- HookRunReaper marks orphaned/failed, then same reporter converges external Check to `timed_out`/`failure`. A real late completion may replace orphaned desired for the same run, still under generation CAS. So may a late review: authorization, the attempt reservation and the result report accept a run only the reaper ended, under the rest of their fence (#2247); `hook/start` still requires a running run.
...
Generation CAS cannot retract an in-flight HTTP request, so external effects use durable mutex. While `writePhase + writeMarker` exists, no new worker writes even after lease expiry; takeover performs GET/list reconciliation first. New revision/rerun writes `pendingIntent` rather than switching generation. Each request carries readable marker (`details_url`/`target_url` + normalized output). Only after observing marker **and its original state** remotely may worker clear mutex, fold pending intent, increment generation, and send next. Never blindly retry uncertain Check create/PATCH. While GitHub may still be lagging, reconciliation keeps the mutex and backs off. Once the write is older than a grace window (10 min) and GitHub returns the Check (update) or a complete name listing (create) without the marker, the request provably never took effect — typically a worker killed between the durable marker and the send — so the mutex is released and the next pass mints a fresh marker and re-issues the same desired state (`ambiguous_write_reissued`). A capped listing, or a marker whose encoded state cannot be decoded, keeps the mutex indefinitely. This prevents stale request from turning green after new queued/failure. Required acceptance must also test irreducible GitHub event-delay window before webhook arrival.
...
R2a informational names use a fixed human-readable prefix plus the immutable agent slug; console does not manage them as required. An operator can require one of these Checks, and a terminal review without a formal verdict now fails it, but R2a still cannot guarantee a Check for every revision or fork PR; the supported required gate remains R2b work. Correlation uses `external_id + write marker`, keeping internal hook IDs out of final `name`/`output`. The agent qualifier keeps parallel review hooks on one SHA visible as independent Checks. Required context remains unique/immutable and pins expected source to AgentConnect App. required→informational/off, disable, HookDef/Agent delete first tombstone + queue non-passing cleanup for current required context, then informational uses its own name; console warns operator to remove branch rule. Transport/gate switches change context and never implicitly reuse.
...
- Non-code label/comment turn success never touches required context. A successful structured review — a generation's, or a conversation turn's verdict amendment — can update the informational verdict, but a same-SHA turn cannot reopen required context. R2b decides amendments and same-SHA rerequests together; an amendment is the easier case, since it writes one terminal state and has no queued phase that could leave an earlier `success` standing.
...
| Agent turn ended without a formal review result    | `failure`         | `failure`     |
| Agent unavailable before dispatch                  | `failure`         | `failure`     |
| Turn interrupted by an agent handover              | `failure`         | `failure`     |
...
An ended informational generation without a submitted verdict is `failure`; a submitted `COMMENT + neutral` remains `neutral`. Required generation never becomes `success` merely because the model finished; `COMMENT + neutral` in required maps to `action_required`/`failure` and cannot open the merge gate.

An ordinary, unmentioned PR conversation with `review_action_only` is not a new review generation. With no formal result or only `COMMENT + neutral`, current revision projection remains unchanged; later Q&A cannot wash existing `action_required` or `success` into neutral. Such a turn may instead **amend** the sealed verdict under decision 1, item 5 — same head, a verdict that changes, the same trigger-role bar as a rerequest — and that amendment lands as a fresh terminal generation. An explicit Agent/App mention on an integration that enables formal reviews is a human review request: it opens a generation on the same SHA and enters `in_progress` at `hook/start`. With review policy `off`, the same mention is a non-review activation: it cannot publish a formal review and does not choose a different reply/output path. Inheritance occurs only within the same `reportSha` projection and never carries old-SHA approval to a new SHA.

Preparation and analysis are not conclusions. Check stays `status=in_progress` without `conclusion`/`completed_at` and displays `Preparing review` until `hook/start` advances it to `Analyzing this revision`. Only a terminal review result or terminal no-verdict outcome updates to `completed`; neutral never means ongoing.

Use `failure` for a terminal turn without formal verdict and relay-confirmed unavailability before dispatch (internally retaining the closed retryable reason). The `HookRun.status` keeps its actual operational outcome. Quota exhaustion is titled `Review could not be completed: provider usage limit reached`; other failures can say `Review could not be completed`, and unavailable agents say `Agent unavailable`, hiding topology. Submitted formal verdict takes precedence over runtime terminal. Review-write failure/uncertainty remains `failure`; reaper timeout remains `timed_out`. Every active Check completed in a terminal state carries a `Request review` action; tombstoned cleanup Checks do not advertise an action they can no longer execute.

The retryable delivery-stage classification occurs only after daemon-local durable receipt replay. A retained live row or terminal receipt returns `accepted` before duty, pause, or drain gates, so a lost relay report cannot turn a prior admission into a side-effect-free refusal. A configured platform anchor send also counts as a possible effect once attempted: if drain begins before inbox admission, the daemon reports nonretryable `anchor_side_effect` even when the provider response was lost. Retryable refusal ACKs stay out of the daemon's terminal ACK cache so the same GUID can be re-evaluated after recovery.

An admitted turn the daemon itself ended because it stopped serving the agent reports the normalized `agent_handover` instead of the interrupt vocabulary a user `!stop` produces (`packages/protocol/src/frames/hook.ts`). That is the duty teardown specifically — a revoke, a self-fence on an unrenewed lease, or a registry change that leaves the agent in no held group — not every way a turn can be cut short: a graceful shutdown interrupts the same way but deliberately reports nothing, retaining the row for the SAME daemon's restart replay, and a drain cancels through its own paths and keeps their reasons. That retention stops where the identity does — a duty-governed member releases its duties as it drains, and a handed-off hook row reports even mid-drain, because only the accepted `dispatchDaemonId` could ever re-run it. A verdict already submitted still takes precedence as above; without one the Check fails, but the interruption must not borrow the runtime-failure title. Nothing about the revision was judged, so the title is the call to action `Comment @<app-slug> to retry the interrupted review` (falling back to `Review was interrupted before it finished`) and the summary adds a `How to run this review again` section pointing at the always-present `Request review` action. Recovery is that maintainer action, not an automatic re-fire: the run crossed `hook/start`, so it is outside the delivery-stage retry set, and its own durable delivery receipt makes a same-GUID redelivery a no-op at the daemon.
...
`failure` informational Check carrying a `Request review` requested action. Because
...
- Relay distinguishes issue vs PR comment from payload. Check request/rerequest and workflow approval use separate metadata-only CP resolution + current-rule fences; App reviewer request uses the same live trigger-role gate as thread comments (admin/write/triage or a Trusted user). None depends on ordinary subscription cadence.
- `hook/start/ok` carries an optional `amendment` naming the sealed verdict a conversation turn may change; an older CP omits it and an older daemon ignores it, so both directions default to no authority.
...
- A conversation turn whose `hook/start/ok` names an amendment gets a `Verdict amendment available` block seated after prompt assembly (the current verdict, the one change the policy allows, and the verify-at-the-revision rule); `submitGithubReview` refuses a same-verdict or neutral amendment before spending the turn's attempt, and an inline reply's thread post is not suppressed by the submitted amendment.
...
- `latestReviewVerdictRun(hookId, {repoId, projectionEpoch, pullNumber, reportSha})` is the sealed-verdict lookup behind both the `hook/start` amendment (evaluated on the started row) and the `review-authorize` gate for `review_action_only` runs; a reconciliation of the run's own reserved attempt skips the gate it already passed.
...
- **Reporter:** queued→preparing→in_progress→completed; CP/daemon restart; reaper timeout; durable `preparingAt`/`turnStartedAt` + acknowledged start barrier; completion-first/delayed accepted/preparing/start monotonic CAS; periodic HookRun repair; 429/5xx/permission retry; crash after create before ID persist; `external_id` scan; unique projection generation/CAS; explicit rerun completed Check; old completion fence; terminal seal/nonterminal CAS; stale generic terminal cannot overwrite submitted formal verdict; `projectionEpoch` rebind; marker/upsert/tombstone row-lock races; terminal commit→PR full pagination, fork-head base-repository fallback, partial page, no-current/stale/shared-head block; hook-delete tombstone.
...
| Agent turn/runtime failure                                | No completed formal review; daemon preserves actionable error and session transcript                                                                                                                           | HookRun remains failed; Check ends `failure` with session link and a quota-specific title when applicable                                           |
| Agent unavailable before dispatch                         | Relay sends no turn; internal placement/connection failure retained for operations                                                                                                                             | Periodic repair converges `failure`, title only `Agent unavailable`; no auto-rerun                                                                  |
| Agent stops being served mid-turn                         | Admitted turn is interrupted as a handover and reports `agent_handover`; suppression stops the fallback comment, while a verdict already submitted keeps its own result                                        | `failure` with the interrupted-review title and a `How to run this review again` summary, unless a submitted verdict already owns the projection    |
