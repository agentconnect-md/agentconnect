...
Preparation and analysis are not conclusions. Check stays `status=in_progress` without `conclusion`/`completed_at` and displays `Preparing review` until `hook/start` advances it to `Analyzing this revision`. Only terminal review result updates to `completed`; neutral never means ongoing.
