...
> CP-owned informational Checks. **R2b required gates, R2c fork/merge queue
> support, and R3 commit statuses are not implemented.** The server fails
...
| R2c   | Not implemented | Fork PRs and merge queue / `merge_group`                                                                         |
| R3    | Not implemented | Legacy commit-status transport and `statuses:write` rollout                                                      |

Wire/schema retain `required` and `status` enum values for rolling compatibility, but they are not delivered capabilities. HTTP create/update returns a semantic conflict for both, and daemon/worker fail closed again. R2a accepts `check_run.rerequested`, `check_suite.rerequested`, first-run `check_run.requested_action(request_review)`, and `pull_request.review_requested`. These explicit events may start a new generation only for a current **informational** hook, and the explicit requester must still have repository write/admin permission. Required-safe semantics remain R2b.
...
- R2a retains `required` in schema but rejects it server-side until every R2b release gate passes. Minimum server-verifiable conditions are unconditional coverage of `pull_request:opened|synchronize|ready_for_review|converted_to_draft` (or equivalent `*`), `enabled=true`, `reviewPolicy != off`, `reportingMode != off`, `mentionOnly=false`, and `labelFilter=[]`. R2b permits only `reportingMode=check`; status needs R3's own required acceptance gate. Current contract silences all `edited`, including base retarget, so required cannot ship without a trusted base-retarget revision source. Current App permissions cannot reliably read all branch-protection/ruleset/merge-queue facts, so editor must explicitly acknowledge strict-up-to-date, no merge queue, and fork support assumptions in `requiredAcknowledged*`. Do not market auto-detection. Disable/delete and mode changes show blocking warnings.
...
3. Broker first uses purpose token to `GET pull` again and requires PR not closed/merged/draft and current head/base equal expected revision; otherwise reject and tell agent to reread diff. Then `POST review` with explicit `commit_id=expectedHeadSha`; inline comments and verdict use one batched request.
...
HookRun additionally stores `projectionId/projectionGeneration`, `projectionEpoch`, `projectionIntent`, policy/report snapshots, `pullNumber/head/base/reportSha`, relay-ingest `startedAt`, daemon-dispatch `turnStartedAt`, TEXT `reviewId`, `reviewAttemptId/reviewAttemptState`, `reviewEvent/verdict`, and dispatch fence. Projection is GitHub commit/context granularity; subject rows hold PR revision. Unique key + transactional `generation` increments ensure one external current generation per hook/repository/report SHA. Every worker job carries `(projectionId, generation)` CAS; old epoch/generation can update history only.
...
An explicit review generation on same `reportSha` reuses one Check Run ID, first sets `queued`, then runs agent. It does not temporarily mark old run passing neutral or rely on GitHub selecting newest among duplicate names. This rerun is **informational-only**. In required mode, if previous remote state may be success, a local blocked/queued intent cannot retract an already-sent or uncertain PATCH and would fail open. R2b may remove global 409 only when every supported lifecycle event produces a fresh test-merge reportSha or a live GitHub test proves the event atomically makes old Check non-passing before webhook delivery. Do not create-many as fallback. R3 status has no update ID; each generation targets pending + terminal and is not R2a behavior.
...
Operator still confirms strict "branch up to date." Advancing base tip sends no `pull_request:edited` per open PR; strict protection blocks stale head, and update branch creates `synchronize` new head/test-merge review. Target retarget also only produces currently silenced `pull_request:edited`; required remains unavailable until trusted base-retarget source exists. Merge queue later uses `merge_group` synthetic SHA as `reportSha`. Until real repository E2E passes, all remain informational.
...
- `external_id=projection.id` is recovery correlation, **not GitHub idempotency**. For rolling upgrade/rollback, create POST retains legacy recovery name `agentconnect/info/review/<hookId>`. After persisting `checkRunId`, that PATCH changes the name to the agent-qualified human label, and every later state PATCH also includes that label. It must carry `actions` as well: **GitHub drops requested actions omitted from an update**, and a projection whose first write is already terminal never receives a later state PATCH, so a name-only follow-up would silently strip the buttons the create POST published. Verified directly against a live `skipped` Check — re-asserting `actions` through one PATCH restored the button without changing the conclusion, so the conclusion never suppressed it. Qualifying the name prevents GitHub's latest-per-name filtering from collapsing two review agents on the same revision into one misleading status. Ambiguous create lists runs by report SHA + legacy name + App with pagination, then matches `external_id + write marker`; DB lease/CAS serializes creates. Because it now carries presentation rather than a generation-invariant label alone, that follow-up takes the projection write mutex like any other state write; losing the fence means a newer generation owns the Check and writes its own name and actions, so the stale write is dropped instead of restoring the previous generation's buttons. Its failure still does not roll back the settled projection: the Check's state is the one the create wrote, so the fence is released with that observed value rather than reconciled as an ambiguous state mutation, and a later state PATCH reconverges the name. The ambiguous-create recovery path renames only — it knows just what GitHub observed and must not invent presentation. R2a does not bulk-rename historical settled Checks.
...
a `How to start this review` section naming both entry points and the write/admin
...
- **Fork PR:** GitHub states Checks associate only with pushes in repository where run was created. Whether base-repository Check on fork head appears/satisfies protection requires real fork acceptance test. Informational request action no longer trusts fork-empty `check_run.pull_requests`, but remains not required-compatible until acceptance.
...
- **Filtered hooks:** mention/label filters or missing `synchronize` can leave newest SHA without context; always informational.
...
- Relay distinguishes issue vs PR comment from payload. Check request/rerequest uses separate metadata-only CP resolution + current-rule fence; App reviewer request uses explicit write-permission gate. Neither depends on ordinary subscription cadence.
...
- Durable QueueEntry inbox adds local-only trusted `HookDispatchContext` (delivery, policy, revision, reply/report target). Restart replay reenters hook-aware dispatch and restores active metadata/start/completion; formal review uses hidden marker recovery. Poster keeps the single-attempt contract with `posterPublishState=not_started|in_flight|settled`; after crash, in_flight is unknown and never automatically POST-replayed. Fallback POST is allowed only when no formal attempt exists or the same-ID result for the current/latest attempt is `not_submitted`; submitted/ambiguous/no result/stale mismatch/other unresolved never calls poster and durably settles. A legacy inbox row without context fails/tombstones and relies on the CP reaper; it never silently reruns as a generic turn.
...
| **R2c** | Not implemented      | Fork and merge-group support if acceptance spike proves feasible                                                                                        | Real-repository E2E for external fork and merge-queue synthetic SHA; otherwise explicitly unsupported                                                                                                       |
...
Boundary again: **R2a has no required gate, required-safe rerequest, fork/merge-group support, or commit-status transport**. They belong to R2b, R2c, and R3. `projectionEpoch`, association barrier, and ACK outbox do not imply those capabilities are enabled.
...
- **Future R2b required safety:** missing synchronize/ready_for_review; draft conversion; base advance; base retarget without trusted revision event; strict-up-to-date assumption; label/mention filter; missing/unchanged test-merge SHA; multiple PR shared head; disable/delete; context mode switch; COMMENT+neutral/no verdict; distinct Check/status names; expected App source.
