#!/usr/bin/env node
/**
 * Re-vendors the pinned agent-sandbox release manifest into charts/operator.
 *
 *   node scripts/vendor-agent-sandbox.mjs v0.5.4
 *
 * Upstream publishes release manifests but no chart to any registry, so the
 * chart cannot depend on one — it carries the manifest instead. The single
 * edit made here is `helm.sh/resource-policy: keep` on every CRD, matching how
 * this chart treats its own: no release uninstall may cascade-delete live
 * Sandboxes. Deterministic, so re-running it on the same tag is a no-op diff.
 */
...
const target = join(dirname(dirname(fileURLToPath(import.meta.url))), 'charts/operator/vendor/agent-sandbox.yaml')
...
const header = [
  ...
  '#',
  '# Generated by scripts/vendor-agent-sandbox.mjs — do not edit. The only change',
