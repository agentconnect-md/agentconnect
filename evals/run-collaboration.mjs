...
if (!process.env.AGENTCONNECT_DAEMON_ENTRY) {
  ...
  // Games embed Daemon inside Promptfoo, so process.argv[1] is Promptfoo's
  // entrypoint rather than the AgentConnect CLI. Pin the just-built CLI bundle
  // explicitly so real ACP runtimes spawn the real `mcp-bridge` subprocess.
  process.env.AGENTCONNECT_DAEMON_ENTRY = daemonEntry
}
const extraArgs = process.argv.slice(2).filter((arg, index) => !(arg === '--' && index === 0))
// Real-agent trials (§14 step 4) need the operator's subject template. Without
// one, run only the credential-free scripted gate instead of failing the run.
const realConfigured =
  !!process.env.AGENTCONNECT_EVAL_SUBJECT_ROOT?.trim() && !!process.env.AGENTCONNECT_EVAL_GAME_TEMPLATE_AGENTS?.trim()
const providerFilter =
  realConfigured || extraArgs.includes('--filter-providers') ? [] : ['--filter-providers', 'scripted-hosts']
if (!realConfigured) {
  console.error(
    'Collaboration Arena: no real-agent subject configured (AGENTCONNECT_EVAL_SUBJECT_ROOT / AGENTCONNECT_EVAL_GAME_TEMPLATE_AGENTS) — running the scripted-hosts gate only.'
  )
}
