...
export function GateChainFields({
  ...
  const { path, enter, to, back } = useDecisionChainPath(value, onChange)
  const nameOf = (decisionId?: string) => decisions.find((d) => d.id === decisionId)?.name ?? t('missing')
  // One level of the chain: the root's branches in place, or a later Decision inside its sheet.
    ...
    return (
      <div className="flex flex-col gap-3">
        ...
        {(['nextStepId', 'elseStepId'] as const).map((key) => {
          ...
          const next = value.steps?.find((entry) => entry.id === id)
  ...
  const nodes: DecisionGateStep[] = [value, ...(value.steps ?? [])]
  return (
    <>
      {level()}
      {path.map((id, index) => {
        const parent = nodes.find((node) => node.nextStepId === id || node.elseStepId === id)
        return (
          <DecisionChainSheet
            key={id}
            depth={index + 1}
            ...
            condition={parent ? t(parent.nextStepId === id ? 'matches' : 'misses') : undefined}
            title={nameOf(value.steps?.find((entry) => entry.id === id)?.decisionId)}
            canWrite={!disabled}
            onParent={() => to(index)}
            onSave={() => back(true)}
            onCancel={() => back(false)}
