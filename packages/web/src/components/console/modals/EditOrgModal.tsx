...
export default function EditOrgModal({ onClose }: { onClose: () => void }) {
  ...
  const t = useTranslations('Settings.organization.editDialog')
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (e instanceof ApiError && e.status === 409) setErr(t('slugTaken'))
  ...
  const remove = async () => {
    ...
    try {
      ...
      if (e instanceof ApiError && e.status === 409) setErr(t('hasDaemons'))
      else setErr(e instanceof Error ? e.message : String(e))
  ...
  return (
    <>
      ...
      <div className="modalfoot">
        ...
        <Button onClick={() => void submit()}>{busy ? t('saving') : t('save')}</Button>
      </div>
    </>
  )
}
