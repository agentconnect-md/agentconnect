...
// otherwise it is display-only. Two sizes match the design: the compact "Outdated" chip on
// the daemon list card, and the "Update to <latest>" chip in the detail header. Renders
// nothing unless an upgrade is actually available.
...
export function DaemonUpgradeBadge({
  ...
  const label = md ? `Update to ${latest}` : 'Outdated'
