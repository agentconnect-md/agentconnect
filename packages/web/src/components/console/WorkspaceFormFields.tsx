  ...
  return (
    ...
    >
      {ACCESS_TOGGLE.map((option) => {
        // Lowering is revoke and authorize again, so read is offered only to a row that has it.
        const lowers = option.value === 'read' && value !== 'read'
        return (
          <button
            key={option.value}
            type="button"
            ...
            title={lowers ? t('lowerAccessRevoke') : t(option.label)}
            disabled={disabled || lowers}
            className={
              value === option.value
                ? 'pill on flex h-[22px] w-[26px] cursor-default items-center justify-center p-0'
                : lowers
                  ? 'pill flex h-[22px] w-[26px] cursor-not-allowed items-center justify-center p-0 opacity-50'
