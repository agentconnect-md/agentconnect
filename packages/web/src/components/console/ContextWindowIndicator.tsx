...
export function ContextWindowIndicator({ used, size }: { used?: number; size?: number }) {
  const t = useTranslations('Common.contextWindow')
    ...
    <span className="group relative inline-flex flex-none">
      ...
      >
        ...
        <span className="mt-[3px] block font-sans text-[12px] font-medium leading-normal text-(--text-primary)">
          {percentage == null ? t('usageUnavailable') : t('percentage', { percentage, left: 100 - percentage })}
        </span>
        {usedText && (
          <span className="mt-[3px] block font-sans text-[12px] font-medium leading-normal text-(--text-primary)">
            {sizeText ? t('tokensWithSize', { used: usedText, size: sizeText }) : t('tokens', { used: usedText })}
          </span>
        )}
      </span>
    </span>
