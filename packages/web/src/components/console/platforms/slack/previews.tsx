...
export function SlackManifestPreview() {
  const t = useTranslations('Platforms.slack.previews')
  return (
      ...
      <div className="rounded-xl border border-[#e0e0e2] bg-white p-3 shadow-(--shadow-xl)">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="relative rounded-lg bg-white p-2.5">
            ...
            <div className="font-sans text-[11.5px] font-bold leading-tight text-[#1d1c1d]">{t('manifestTile')}</div>
            <div className="mt-0.5 font-sans text-[10px] leading-tight text-[#616061]">{t('manifestTileDesc')}</div>
          </div>
          <div className="rounded-lg border border-[#e0e0e2] bg-white p-2.5">
            <span className="mb-1.5 flex h-6 w-6 items-center justify-center rounded-md bg-[#f4f4f4] text-[#454545]">
              <Icon name="clapperboard" size={14} />
            </span>
            <div className="font-sans text-[11.5px] font-bold leading-tight text-[#1d1c1d]">{t('blankTile')}</div>
            <div className="mt-0.5 font-sans text-[10px] leading-tight text-[#616061]">{t('blankTileDesc')}</div>
          </div>
        </div>
        <div className="mt-2 font-sans text-[10px] leading-snug text-[#616061]">{t('pasteStep')}</div>
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-(--border-subtle) bg-(--surface-app)">
        ...
        <div className="h-[140px] overflow-hidden bg-(--surface-card)">
          <div className="cfg-scroll px-2.5 py-2">
            <div className="mb-1.5 font-sans text-[10px] font-bold leading-tight text-(--text-primary)">
              {t('yourApps')}
            </div>
            {[0, 1].map((i) => (
              <div
            ...
            <div className="mt-2 rounded-md border border-(--border-subtle) bg-(--surface-app) p-2">
              ...
              <div className="grid grid-cols-[1fr_auto_auto] items-center gap-x-2 gap-y-1">
                ...
                <span className="font-mono text-[9px] leading-normal text-(--text-secondary)">
                  {t('workspaceValue')}
                </span>
              ...
              <div className="mt-1 font-sans text-[8px] leading-normal text-(--text-tertiary)">{t('expires')}</div>
            </div>
          </div>
        </div>
      </div>
      <div className="mt-1.5 px-1 font-sans text-[10.5px] font-normal leading-[1.45] text-(--text-secondary)">
        {t.rich('scrollHint', { apps: (chunks) => <span className="mono">{chunks}</span> })}
      </div>
    </div>
  )
}
