 ...
 * WebSocketServer (the accept side of a wire).
 *
 * The application heartbeat is one-directional, so a half-open TCP connection —
 * peer gone without a FIN/RST (a node crash, an edge/middlebox dropping the
 * flow) — leaves the server holding a socket it believes is live. This sweeps
 * every `intervalMs`: a socket with no inbound activity since the last sweep is
 * `terminate()`d (→ its `close` fires → the connection FSM drops it); the rest
 * are pinged. The peer's ws layer answers pings automatically (and browsers pong
 * at the protocol level), so no peer-side code is required. A socket is torn down
 * after at most ~2 intervals of silence.
 *
 * Because these gateways run in `noServer` mode they never emit `'connection'`,
 * so the caller must `track()` each socket it accepts (inside its `handleUpgrade`
 * callback). Returns that `track` fn; the sweep timer is `unref`'d and
 * auto-cleared when the server closes.
 *
 * Shared by every accept-side gateway: the relay's daemon-facing `rd/*` and
 * browser-facing servers, plus the CP's daemon and relay control gateways.
 */
