    ...
    r.get(
      '/bots/:id/decision-routing/evaluations/:seq',
      {
        schema: {
          ...
          description:
            "One routing evaluation with its frozen Decision and routing snapshot, target constraint (agents, participation, and mention or implicit only), input and history, answer, model, usage, the raw provider request and response JSON (from daemons that support it, each capped at 16K characters), and every target's admission status while the host still retains the bodies (bounded to 64 KiB); once stripped, `detailsExpired` is true and only the summary and snapshot remain. Proxied from the evaluation host, never stored or logged, and returned only after the conversation audience check; before any session there names an audience the bodies need edit access to the conversation's owner agent. Returns 404 when the evaluation is gone or unreadable and 503 when the host is offline or must be upgraded, including a reply that names no namespace.",
