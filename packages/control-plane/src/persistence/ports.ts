...
export interface SessionRepo {
  ...
  /** The newest top-level session any of these agents holds in one conversation namespace; a null tenantScope matches only null rows. */
...
export interface AgentRepoAuthorizationRepo {
  ...
  /** Change a grant's tier; lowering a github one throws AgentRepoIntegrationConflict while an enabled review or Checks hook needs it. */
...
export interface AgentInstallationAuthorizationRepo {
  ...
  /** Change the tier and/or materialization in one transaction; null when the row is gone, AgentRepoIntegrationConflict for a tier a covered hook needs. */
  update(
    id: string,
