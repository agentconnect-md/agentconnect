/**
 * The daemon's own Kubernetes identity — the credential an operator-provisioned daemon
 * presents to the control plane instead of an API key
 ...
 * ServiceAccount token into the pod, and presenting it proves which namespace — and so
 * which org — this daemon belongs to. The file is read at every connect rather than once
