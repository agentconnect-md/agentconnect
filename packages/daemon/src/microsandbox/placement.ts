...
import { agentHostKey, hostKeyDirName, type HostKey } from '../acp/host-key.js'
...
export function localMicrosandboxPlacement(input: {
  ...
  if (input.hostKey && input.legacy) return { id: `${input.agentId}/agent`, homeKey: agentHostKey(input.agentId) }
  return { id: `${input.agentId}/${hostKeyDirName(input.hostKey)}` }
}
