/**
 * `agentconnect git-credential --agent <id>` — the git credential helper
 * (docs/designs/github-app-git-credentials.md §Local Helper Channel).
 *
 * Speaks git's credential-helper protocol on stdin/stdout and proxies to the
 * daemon's gitcred.sock with the runtime-only agent capability; the token
 * exists only on the reply pipe. Actions:
