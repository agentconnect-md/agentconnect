...
// Quiesces this process's own git; the bare's repack is disarmed in its config, not here.
function git(cwd: string, args: string[]): string {
  const quiesced = ['-c', 'maintenance.auto=false', '-c', 'gc.auto=0', ...args]
  return execFileSync('git', quiesced, { cwd, env: fixtureEnv(), encoding: 'utf8' })
}

/** Built once per shape and copied per caller; outside `roots`, which `afterEach` empties. */
const templates = new Map<string, string>()
const templateRoot = mkdtempSync(join(tmpdir(), 'ac-secondary-templates-'))

/** Six git subprocesses. Worth paying once per shape rather than once per caller. */
function buildTemplate(key: string, branch: string, gitmodules?: string): string {
  const root = mkdtempSync(join(templateRoot, 'shape-'))
  const bare = join(root, 'origin.git')
  const seed = join(root, 'seed')
  git(root, ['init', '-q', '--bare', `--initial-branch=${branch}`, bare])
  // In the bare's own config, because `-c` cannot reach it: git strips GIT_CONFIG_PARAMETERS when
  // it connects to a local repository, so the `receive-pack` the push below spawns never sees it.
  // That repack detaches and outlives the push, and a template is read ~78 times while it runs —
  // `cpSync` walked an `objects/` being rewritten under it and failed with ENOENT.
  git(bare, ['config', 'receive.autogc', 'false'])
  mkdirSync(seed, { recursive: true })
  git(seed, ['init', '-q', `--initial-branch=${branch}`, '.'])
  writeFileSync(join(seed, 'README.md'), `${key}\n`)
...
// A copy of the template for its shape: this file asks for ~78, at six git subprocesses each.
// Still a repository of the caller's own — several tests push to the one they are handed.
// `name` reaches only the temp-dir prefix now; nothing reads what it used to write.
function bareRepo(name: string, branch: string, gitmodules?: string): string {
  const key = `${branch}\u0000${gitmodules ?? ''}`
  let template = templates.get(key)
  if (template === undefined) {
    template = buildTemplate(key, branch, gitmodules)
    templates.set(key, template)
  }
  const bare = join(tempRoot(`ac-secondary-${name}-`), 'origin.git')
...
function serveAll(agent: Agent, branches: Record<string, string>, gitmodules: Record<string, string> = {}): void {
  ...
  for (const root of workspaces.secondaryRoots(agent)) {
    ...
    serve(root.cloneUrl, bareRepo(root.repoFullName.replaceAll('/', '-'), branch, gitmodules[root.repoFullName]))
  }
}
...
/** The attestation at one subtree, named as it sits under `repos/` (`owner/repo`, or `_gitlab/<id>`). */
function materialization(agent: Agent, subtreeName: string): unknown {
  const [owner, repo] = subtreeName.split('/')
  return JSON.parse(
...
describe('secondaryRoots (layout)', () => {
  ...
  it('lays a GitLab project out by numeric id under `_gitlab`, cloned from the spec’s own instance', () => {
    const project = { repoFullName: 'example-group/sub/example-project', repoId: '4455667', provider: 'gitlab' }
    // A GitHub repository numbered the same: the hosts number theirs independently, so both are roots.
    const agent = agentFixture([project, { repoFullName: 'acme/infra', repoId: '4455667' }])
    const home = workspaces.agentRootFor(agent)
    ...
    expect(
      workspaces.secondaryRoots(agent).map((root) => ({
        ...
        subtreeName: root.subtreeName,
        path: root.path,
        worktreesPath: root.worktreesPath,
        cloneUrl: root.cloneUrl,
        managedHost: root.managed?.host.baseUrl
      }))
    ).toEqual([
      {
        repoFullName: 'acme/infra',
        ...
        worktreesPath: join(home, 'repos', 'acme', 'infra', 'worktrees'),
        cloneUrl: 'https://github.com/acme/infra',
        managedHost: 'https://github.com'
      },
      {
        repoFullName: 'example-group/sub/example-project',
        provider: 'gitlab',
        subtreeName: '_gitlab/4455667',
        path: join(home, 'repos', '_gitlab', '4455667', 'checkout'),
        worktreesPath: join(home, 'repos', '_gitlab', '4455667', 'worktrees'),
        // GitLab's `.git` rule, and the subgroup path kept whole — neither is github canonicalization.
        cloneUrl: 'https://gitlab.com/example-group/sub/example-project.git',
        managedHost: 'https://gitlab.com'
      }
    ])

    // A self-managed instance (§24.4), path prefix and all: the address AND the credential pin follow it.
    const hosted = agentFixture([project], { gitlabHost: 'https://gitlab.example.test:8443/gitlab/' })
    expect(workspaces.secondaryRoots(hosted)[0]).toMatchObject({
      cloneUrl: 'https://gitlab.example.test:8443/gitlab/example-group/sub/example-project.git',
      githubApp: true,
      managed: {
        host: { provider: 'gitlab', baseUrl: 'https://gitlab.example.test:8443/gitlab' },
        gitlabHost: 'https://gitlab.example.test:8443/gitlab/',
        gitlabRepoBearing: true
      }
    })
  })

  it('refuses a console selector that names a repository on both hosts, whatever its case', async () => {
    // The selector carries no provider, and the browser already matched it case-insensitively, so
    // exact casing proves nothing: a name both hosts hold is refused rather than routed to either.
    const agent = agentFixture([
      { repoFullName: 'acme/shared', repoId: '1' },
      { repoFullName: 'Acme/shared', repoId: '2', provider: 'gitlab' },
      { repoFullName: 'acme/infra', repoId: '3' },
      { repoFullName: 'acme/infra', repoId: '4', provider: 'gitlab' }
    ])

    expect(await workspaces.consoleSecondaryRoot(agent, 'acme/shared')).toBeUndefined()
    expect(await workspaces.consoleSecondaryRoot(agent, 'Acme/shared')).toBeUndefined()
    expect(await workspaces.consoleSecondaryRoot(agent, 'acme/infra')).toBeUndefined()
    // Case-insensitive as before when only one host holds the name.
    expect(
      await workspaces.consoleSecondaryRoot(agentFixture([{ repoFullName: 'acme/infra', repoId: '3' }]), 'ACME/INFRA')
    ).toMatchObject({ repoId: '3' })
  })

  it('refuses a GitLab row that cannot place a subtree, and a GitHub row squatting the `_gitlab` owner', () => {
    const agent = agentFixture([
      { repoFullName: 'group/project', repoId: '0', provider: 'gitlab' },
      { repoFullName: 'group/project', repoId: '../1', provider: 'gitlab' },
      { repoFullName: 'project', repoId: '2', provider: 'gitlab' },
      { repoFullName: 'group/../project', repoId: '3', provider: 'gitlab' },
      { repoFullName: '_gitlab/4', repoId: '4' },
      { repoFullName: 'group/project', repoId: '5', provider: 'gitlab' }
    ])

    expect(workspaces.secondaryRoots(agent).map((root) => root.subtreeName)).toEqual(['_gitlab/5'])
  })
})
...
describe('secondary root materialization', () => {
  ...
  it('materializes a GitLab project under `_gitlab/<id>` and hands it out under its own path', async () => {
    const project = { repoFullName: 'example-group/sub/example-project', repoId: '4455667', provider: 'gitlab' }
    const agent = agentFixture([project])
    serveAll(agent, { 'example-group/sub/example-project': 'develop' })

    const cwd = await workspaces.prepareWorkspace(agent)

    const checkout = join(workspaces.agentRootFor(agent), 'repos', '_gitlab', '4455667', 'checkout')
    expect(existsSync(join(checkout, '.git'))).toBe(true)
    expect(git(checkout, ['rev-parse', '--abbrev-ref', 'HEAD']).trim()).toBe('develop')
    expect(materialization(agent, '_gitlab/4455667')).toEqual({
      provider: 'gitlab',
      repoId: '4455667',
      repoFullName: 'example-group/sub/example-project',
      branch: 'develop'
    })
    // Agent-run git in this checkout authenticates on the GitLab instance, never on github.com.
    expect(git(checkout, ['config', '--get-all', 'credential.https://gitlab.com.helper'])).toContain(SHIM)
    expect(git(checkout, ['config', '--get', 'credential.https://gitlab.com.useHttpPath']).trim()).toBe('true')
    expect(() => git(checkout, ['config', '--get-all', 'credential.https://github.com.helper'])).toThrow()
    // The session gets the directory, and its standing context names it by the project's own path.
    const ready = await workspaces.readySecondaryRoots(agent)
    expect(ready.map((root) => [root.repoFullName, root.branch])).toEqual([
      ['example-group/sub/example-project', 'develop']
    ])
    expect(await workspaces.additionalWorkspaceDirectories(agent, cwd)).toEqual([realpathSync(checkout)])
    expect(buildWorkspaceRootsAppend(ready)).toContain(
      `- ${realpathSync(checkout)} — example-group/sub/example-project (develop)`
    )
  })
...
describe('retire → sweep → remove (decision 12)', () => {
  ...
  it('retires a subtree whose repository id is no longer authorized', async () => {
    ...
    expect((await workspaces.retiredSecondaryRoots(after)).map((root) => root.subtreeName)).toEqual([
      'example-co/shared-library'
    ])
  ...
  it('retires the directory a rename moved the repository out of, and re-authorizing un-retires in place', async () => {
    ...
    expect((await workspaces.retiredSecondaryRoots(renamed)).map((root) => root.subtreeName)).toEqual(['acme/infra'])
  ...
  it('keeps a GitLab root in place across a rename, and retires it only once its own id leaves the set', async () => {
    const agent = agentFixture([
      { repoFullName: 'example-group/example-project', repoId: '4455667', provider: 'gitlab' }
    ])
    serveAll(agent, { 'example-group/example-project': 'main' })
    await workspaces.prepareWorkspace(agent)
    restoreAuthorizedOrigins(agent)
    const checkout = join(workspaces.agentRootFor(agent), 'repos', '_gitlab', '4455667', 'checkout')

    // The subtree is keyed by id, so a new path is the same directory — with a new origin to converge onto.
    const renamed = reauthorized(agent, [
      { repoFullName: 'example-group/renamed-project', repoId: '4455667', provider: 'gitlab' }
    ])
    expect(await workspaces.retiredSecondaryRoots(renamed)).toEqual([])
    serve(
      workspaces.secondaryRoots(renamed)[0]!.cloneUrl,
      remotes.get('https://gitlab.com/example-group/example-project')!
    )
    await workspaces.prepareWorkspace(renamed)
    expect((await workspaces.readySecondaryRoots(renamed)).map((root) => root.repoFullName)).toEqual([
      'example-group/renamed-project'
    ])
    expect(git(checkout, ['remote', 'get-url', 'origin']).trim()).toBe(
      'https://gitlab.com/example-group/renamed-project.git'
    )

    // A GitHub repository numbered the same is not this root, so the row's departure still retires it.
    const swapped = reauthorized(agent, [{ repoFullName: 'acme/infra', repoId: '4455667' }])
