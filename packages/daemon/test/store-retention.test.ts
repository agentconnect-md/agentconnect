...
function seedEveryTable(s: LocalStore, agentId: string, tag: string, at: number, owner: string): void {
  ...
  expect(s.completeHookInbox(hookId, JSON.stringify({ id: hookId }), at, owner)).toBe('completed')
  s.upsertSession({
    key: tag,
    agentId,
    platform: 'slack',
  ...
  s.deleteSession(tag, { reason: 'retention', at, ownerId: owner })
  // Written after the delete: `deleteSession` drops the session's own outbox snapshot with it.
  s.saveSessionMetadataSnapshot(agentId, `acp-${tag}`, '{"phase":"end"}', true, at, owner)
  ...
  expect(s.expireActivations(at).transcriptOnly.map((row) => row.activationKey)).toContain(`act-${tag}`)
  s.recordRuntimeCatalogMeta({ runtimeId: `runtime-${tag}`, fingerprint: 'fp', source: 'acp', observedAt: at })
  s.upsertRuntimeModelCap({
...
const remaining = (s: LocalStore, id: string): number => s.listRetentionCandidates(rule(id)).length
...
describe('store retention rule table', () => {
  it('runs every declared rule against the real schema, on either backend', async () => {
    ...
    const s = solo()
    seedEveryTable(s, LIVE, 'fresh', AT, 'member-a')
  ...
  it('collects every rule’s row once its own horizon has passed, and reports them per rule', async () => {
    const s = solo()
    seedEveryTable(s, LIVE, 'old', AT, 'member-a')
    ...
    expect(remaining(s, 'hook-report')).toBe(0)
    expect(remaining(s, 'session-purge')).toBe(1)
    ...
    expect(remaining(s, 'session-purge')).toBe(0)
    expect(remaining(s, 'catalog-meta')).toBe(0)
    s.close()
  })

  it('keeps a row one millisecond short of its horizon, and scales every horizon at once', async () => {
    const s = solo()
    seedEveryTable(s, LIVE, 'edge', AT, 'member-a')
  ...
  it('reclaims a departed writer’s cache on the shorter window and keeps its own on the long one', async () => {
    ...
    const s = solo()
    seedEveryTable(s, LIVE, 'mine', AT, 'member-a')
  ...
  it('keeps a refreshed catalog whole, the models discovery found included', async () => {
    ...
    const s = solo()
    s.recordRuntimeCatalogMeta({ runtimeId: 'claude', fingerprint: 'fp-1', source: 'acp', observedAt: AT })
    s.upsertRuntimeModelCap({ runtimeId: 'claude', modelId: 'opus', fingerprint: 'fp-1', caps: {}, observedAt: AT })
    s.upsertRuntimeModelCap({ runtimeId: 'claude', modelId: 'sonnet', fingerprint: 'fp-1', caps: {}, observedAt: AT })
    s.markRuntimeCatalogComplete('claude', 'fp-1', 'hash-1', AT)
    // Only the meta row and the seed model are re-stamped, 20 days later.
    s.recordRuntimeCatalogMeta({ runtimeId: 'claude', fingerprint: 'fp-1', source: 'acp', observedAt: AT + 20 * DAY })
    s.upsertRuntimeModelCap({
      runtimeId: 'claude',
      modelId: 'opus',
      fingerprint: 'fp-1',
  ...
  it('reclaims a departed member’s catalog without touching a live peer’s', async () => {
    ...
    for (const [member, model] of [
      ...
      member.recordRuntimeCatalogMeta({ runtimeId: 'claude', fingerprint: 'fp', source: 'acp', observedAt: AT })
      member.upsertRuntimeModelCap({ runtimeId: 'claude', modelId: model, fingerprint: 'fp', caps: {}, observedAt: AT })
    }
    ...
    expect(live.getRuntimeCatalogMeta('claude')).toMatchObject({ fingerprint: 'fp' })
  ...
  it('collects a forgotten agent’s rows on the control plane’s answer, in one batched read', async () => {
    ...
    seedEveryTable(a, LIVE, 'live', AT, 'member-a')
    seedEveryTable(a, GONE, 'gone', AT, 'member-a')
    const cp = fakeCp()
    ...
    expect(remaining(b, 'hook-report')).toBe(1) // the live agent's
  ...
  it('leaves a row whose obligation was renewed between the read and the delete', async () => {
    ...
    const [a, b] = sharedMembers('member-a', 'member-b')
    seedEveryTable(a, LIVE, 'renewed', AT, 'member-a')
    const stale = b.listRetentionCandidates(rule('session-metadata'))[0]!

    a.saveSessionMetadataSnapshot(LIVE, 'acp-renewed', '{"phase":"end"}', true, AT + 5_000, 'member-a')

    expect(b.deleteRetentionRow(rule('session-metadata'), stale)).toBe(false)
    expect(b.listRetentionCandidates(rule('session-metadata'))).toHaveLength(1)
    a.close()
  })
  ...
  it('leaves a hook report the owner acknowledged between the read and the delete', async () => {
    ...
    const [a, b] = sharedMembers('member-a', 'member-b')
    seedEveryTable(a, LIVE, 'acked', AT, 'member-a')
    const stale = b.listRetentionCandidates(rule('hook-report'))[0]!

    expect(a.acknowledgeHookInbox('hook-acked', { ownerId: 'member-a' })).toBe(true)

    expect(b.deleteRetentionRow(rule('hook-report'), stale)).toBe(false)
    a.close()
  })
  ...
  it('ages the work, not the attempt, so a row nobody will ever accept still goes', async () => {
    ...
    const [a, b] = sharedMembers('member-a', 'member-b')
    seedEveryTable(a, LIVE, 'stuck', AT, 'member-a')

    // 30 days of failed attempts, the last one moments ago.
    a.claimSessionPurges(LIVE, ['acp-stuck'], 'member-a', AT + 30 * DAY - 1)
    expect(a.claimHookTerminalReport('hook-stuck', 'member-a', AT + 30 * DAY - 1)).toBe(true)

    const summary = await sweeper(b, AT + 30 * DAY).instance.sweep()

    expect(summary!.byRule).toMatchObject({ 'session-purge': 1, 'hook-report': 1 })
    expect(remaining(b, 'session-purge')).toBe(0)
    expect(remaining(b, 'hook-report')).toBe(0)
    a.close() // one database backs the pair
  })

  it('re-arms a session-metadata row when a NEW snapshot replaces it, because that is new work', async () => {
    const s = solo()
    seedEveryTable(s, LIVE, 'live', AT, 'member-a')
    // A fresh snapshot for the same session re-stamps queuedAt: the obligation is new.
    s.saveSessionMetadataSnapshot(LIVE, 'acp-live', '{"phase":"end"}', true, AT + 6 * DAY, 'member-a')
  ...
  it('sweeps synchronously when nobody is asked, which is what startup needs', () => {
    const s = solo()
    seedEveryTable(s, LIVE, 'old', AT, 'member-a')

    const { instance } = sweeper(s, AT + 30 * DAY)
    const summary = instance.sweepAgeOnly()
  ...
  it('reads the horizon scale and the orphan switch from the deployment env', () => {
