...
describe.skipIf(!databaseUrl)('PostgreSQL pool store transcript fence', () => {
  it('isolates two organizations sharing one channel/thread key across pool members', async () => {
    ...
    try {
      ...
      first.store.appendTranscript({ ...row, recipient: agentA, text: 'org A row' })
      second.store.appendTranscript({ ...row, recipient: agentB, text: 'org B row' })
      expect(first.store.transcriptPageForAgent(channel, thread, agentA, null, 10).rows.map((r) => r.text)).toEqual([
        'org A row'
      ])
      expect(second.store.transcriptPageForAgent(channel, thread, agentB, null, 10).rows.map((r) => r.text)).toEqual([
        'org B row'
      ])
      ...
      first.store.insertToolCall({ ...call, sender: agentA, body: '{"rawInput":"A"}' })
      second.store.insertToolCall({ ...call, sender: agentB, body: '{"rawInput":"B"}' })
      second.store.updateToolCall(channel, thread, agentB, 'call-1', { title: 'Bash', body: '{"rawInput":"B2"}' })
      expect(first.store.getToolBodyForAgent(channel, thread, agentA, 'call-1')).toBe('{"rawInput":"A"}')
      expect(second.store.getToolBodyForAgent(channel, thread, agentB, 'call-1')).toBe('{"rawInput":"B2"}')

      // One org's write never moves the other's context fence for the same thread key.
      const peerRevision = second.store.threadTranscriptRevision(channel, thread, agentB)
      first.store.appendTranscript({ channel, thread, ts: '3.000001', sender: agentA, kind: 'text', text: 'A reply' })
      expect(second.store.threadTranscriptRevision(channel, thread, agentB)).toBe(peerRevision)
      expect(first.store.currentTranscriptRevision(agentA)).toBeGreaterThan(peerRevision)
    } finally {
