...
Where each kind of evidence lives, what it can and cannot tell you, and how to read it without changing anything. Table and column names come from the code at the time of writing; when the daemon under investigation runs an older release, confirm the schema at that tag (`git show v<agentVersion>:<path>`).
