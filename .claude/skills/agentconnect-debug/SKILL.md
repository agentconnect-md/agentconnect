...
Start with [references/symptoms.md](references/symptoms.md): it maps a reported symptom to the first query, what the result usually means, and where the fix landed. Then read [references/evidence-sources.md](references/evidence-sources.md) for how to read the source you need. Work outward in this order and stop when the evidence explains the report:
